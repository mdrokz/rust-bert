...
impl XLNetRelativeAttention {
    ...
    fn rel_attention_core(
        ...
        let ac = Tensor::einsum("ibnd,jbnd->bnij", &[&(q_head + &self.r_w_bias), k_head_h],None);
        let bd = self.rel_shift_bnij(
            &Tensor::einsum("ibnd,jbnd->bnij", &[&(q_head + &self.r_r_bias), k_head_r],None),
            ac.size()[3],
        );
        ...
        let attention_vector = Tensor::einsum("bnij,jbnd->ibnd", &[&attention_probas, v_head_h],None);
    ...
    fn post_attention(
        ...
        let mut attention_out = Tensor::einsum("ibnd,hnd->ibh", &[attention_vector, &self.output],None)
            .apply_t(&self.dropout, train);
        if residual {
    ...
    pub fn forward_t(
        ...
        let (output_g, attention_probas_g) = if let Some(g) = g {
            ...
            let (attention_vec_g, attention_probas_g) = match target_mapping {
                Some(target_mapping) => {
                    let q_head_g = Tensor::einsum("mbnd,mlb->lbnd", &[&q_head_g, target_mapping],None);
