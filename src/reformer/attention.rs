...
impl LSHSelfAttention {
    ...
    fn query_per_attention_head(&self, hidden_states: &Tensor) -> Tensor {
        let per_head_query_key = self
            ...
            .transpose(-2, -1);
        Tensor::einsum("balh,ahr->balr", &[hidden_states, &per_head_query_key],None)
    }
    ...
    fn hash_vectors(
        ...
        let rotated_vectors = Tensor::einsum("bmtd,mdhr->bmhtr", &[vectors, random_rotations],None);
    ...
    fn len_norm(&self, input_tensor: &Tensor, epsilon: f64) -> Tensor {
        let variance = (input_tensor * input_tensor).mean_dim(Some([-1].as_slice()), true, input_tensor.kind());
    ...
    pub fn forward_t(
            ...
            & (num_hashes > 1)
        {
            ...
            out_vectors = (out_vectors * probs_vectors).sum_dim_intlist(Some([2].as_slice()), false, out_kind);
        }
